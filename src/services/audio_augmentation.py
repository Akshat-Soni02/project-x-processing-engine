...
try:
    ...
    print("Warning: pydub not found. Install with: pip install pydub")

# --- Setup Logging ---
# In a real app, this would be set up globally, often in main().
# For this example, we set it up here:
...
class AudioAugmentation:
    ...
    def __init__(self, config: Dict):
        ...
        self.pipeline: List[Callable[[bytes], Tuple[bytes, Dict]]] = [
    ...
    def silence_processing(self, audio_bytes: bytes) -> Tuple[Optional[bytes], Dict]:
        """
        Removes silent segments using pydub.split_on_silence.
        ...
        logger.debug(f"Silence params: thresh={self.silence_thresh}dBFS, min_len={self.min_silence_len}ms, keep={self.keep_silence}ms")
        ...
        try:
            ...
            logger.info(f"Silence removal done in {duration:.4f}s. Removed {metrics['time_removed_sec']:.2f}s.")
